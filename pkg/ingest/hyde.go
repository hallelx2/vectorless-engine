...
func (p *Pipeline) generateCandidateQuestions(ctx context.Context, docID tree.DocumentID, profile string) error {
	...
	// Build a parent → children map so we can skip internal nodes (the
	// LLM should answer questions about concrete content, not a parent's
	// abstract summary).
...
func runHyDEWithRetry(ctx context.Context, client llmgate.Client, baseReq llmgate.Request, maxRetries int) ([]string, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			// llmgate.ErrNotImplemented means the configured client is a
			// stub (tests with no LLM wired up). Treat as "no questions"
	...
	// Final parse failure — degrade gracefully (the calling code logs and
	// leaves candidate_questions NULL).
...
// Tolerates code-fence wrappers, leading prose, and the
// {"questions":[...]} vs {"questions":["..."]} variants.
